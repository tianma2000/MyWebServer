...
void sort_timer_lst::add_timer(util_timer *timer){
	...
	/*如果目标定时器的超时时间小于当前链表中所有定时器的超时时间，则把该定时器插入链表头部，作为链表的新节点。否则就需要调用重载函数
	add_timer(util_timer*timer,util_timer*lst_head)，把他插入链表中合适的位置，以保证链表的升序性*/
...
void sort_timer_lst::adjust_timer(util_timer* timer){
	if (!timer) {
		...
		/*如果被调整的目标定时器在链表的尾部，或者该定时器新的超时值任然小于其下一个定时器的超时值，则不需要调整*/
		...
		/*如果目标定时器不是链表的头结点，则将该定时器从链表中去出来，然后插入其原来所在位置之后的部分链表中*/
		else {
...
void sort_timer_lst::del_timer(util_timer * timer){
	...
	/*如果链表中至少有两个定时器，且目标定时器是链表的头结点，则将链表的头结点重置为源头结点的下一个节点，然后删除目标定时器*/
	if (timer == head) {
		head = head->next;
		head->prev = NULL;
		delete timer;
	...
	/*如果链表中至少有两个定时器，且目标定时器是链表的尾结点，则将链表的头结点重置为源头结点的上一个节点，然后删除目标定时器*/
	if (timer == tail) {
		tail = tail->prev;
		tail->next = NULL;
		delete timer;
...
void sort_timer_lst::tick(){
	...
	util_timer* tmp = head;
	/*从头结点开始依次处理每个定时器，知道遇到一个尚未到期的定时器，这就是定时器的核心逻辑*/
	while (tmp) {
