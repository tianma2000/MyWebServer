...
//网站根目录
const char* doc_root = "/home/zy/MyWebServer/root";
...
bool http_conn::write() {
	int temp = 0;
	int addnew = 0;   //m_file_address的偏移量
	if (bytes_to_send == 0) {
	...
	while(1) {
		...
		//正常，更新有要发送的数据，和已经发送的数据
		if (temp > 0) {
			bytes_have_send += temp;
			addnew = bytes_have_send - m_write_index;
		}
		if (temp <= -1) {
			//判断缓冲区是不是满了
			if (errno == EAGAIN) {
				...
				if (bytes_have_send >= m_iv[0].iov_len) {
					//不再继续发送头部消息
					m_iv[0].iov_len = 0;
					m_iv[1].iov_base = m_file_address + addnew;
